(ns vetops.phase-test
  "Unit tests of `vetops.phase` rollout logic."
  (:require [clojure.test :refer [deftest is testing]]
            [vetops.phase :as phase]))
...
(deftest phase-1-appointment-scheduling-only
  (testing "phase 1 allows only appointment-scheduling, requires approval"
    (let [{:keys [disposition reason]} (phase/gate 1 {:op :coordinate-appointment-scheduling} :commit)]
      (is (= :escalate disposition))
      (is (= :phase-approval reason)))
    (let [{:keys [disposition]} (phase/gate 1 {:op :coordinate-boarding-assignment} :commit)]
      (is (= :hold disposition)))))
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety ops"
    (let [{:keys [disposition]} (phase/gate 3 {:op :coordinate-appointment-scheduling} :commit)]
      (is (= :commit disposition)))
    (let [{:keys [disposition]} (phase/gate 3 {:op :coordinate-boarding-assignment} :commit)]
      (is (= :commit disposition)))
...
(deftest hard-hold-always-wins
  (testing "a governor HARD hold stays HOLD regardless of phase"
    (doseq [ph [0 1 2 3]]
      (let [{:keys [disposition]} (phase/gate ph {:op :coordinate-appointment-scheduling} :hold)]
        (is (= :hold disposition)
