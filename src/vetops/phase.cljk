(ns vetops.phase
  "Phase 0->3 staged rollout for the ISIC-750 veterinary activities
  operations-coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-scheduling    -- appointment-slot scheduling
                                      allowed, every write needs human
                                      approval.
    Phase 2  assisted-coordination  -- adds boarding assignment, supply
                                      request, staff shift proposals,
                                      still approval.
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                      `:coordinate-appointment-scheduling`/
                                      `:coordinate-boarding-assignment`/
  ...
  needs a human to actually look at it. `vetops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [vetops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"             :writes #{}                                                                        :auto #{}}
   1 {:label "assisted-scheduling"   :writes #{:coordinate-appointment-scheduling}                                      :auto #{}}
   2 {:label "assisted-coordination" :writes #{:coordinate-appointment-scheduling :coordinate-boarding-assignment
                                                :coordinate-supply-request :schedule-staff-shift-proposal}               :auto #{}}
   3 {:label "supervised-auto"       :writes write-ops
      :auto #{:coordinate-appointment-scheduling :coordinate-boarding-assignment
...
(defn verdict->disposition
  "Map a VetOpsGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
